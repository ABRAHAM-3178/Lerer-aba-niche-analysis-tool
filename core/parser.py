...
def parse_chinese_multi_asin_file(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def find_column(patterns: List[str], fallback: str = None) -> Optional[str]:
        ...
        if fallback:
            for col in df.columns:
                if fallback in col.strip():
                    return col
        return None
    
    search_term_col = find_column(['搜索词'], '搜索词')
    search_rank_col = find_column(['搜索频率排名'], '排名')
    ...
    if not search_term_col or not search_rank_col:
        raise ValueError("未找到搜索词或排名列")
    ...
    for _, row in df.iterrows():
        term = row.get(search_term_col, '')
        rank = row.get(search_rank_col, None)
        if asin_1_col and pd.notna(row.get(asin_1_col)):
            rows.append({
                "searchTerm": str(term),
        ...
        if asin_2_col and pd.notna(row.get(asin_2_col)):
            rows.append({
                "searchTerm": str(term),
                "searchFrequencyRank": rank,
        ...
        if asin_3_col and pd.notna(row.get(asin_3_col)):
            rows.append({
                "searchTerm": str(term),
                "searchFrequencyRank": rank,
...
def parse_aba_csv(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    avg_click = df_clean['click_share'].mean() if 'click_share' in df_clean.columns else None
    avg_conv = df_clean['conversion_share'].mean() if 'conversion_share' in df_clean.columns else None
