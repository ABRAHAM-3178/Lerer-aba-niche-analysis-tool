...
# 只导入 parse_aba_csv，其他函数在 app 内部定义
...
# ============================================================
    ...
    if uploaded_files:
        ...
        if merged_keywords and len(merged_keywords) > 0:
            ...
            # 平均点击份额评级（使用 st.markdown 替代 st.info 避免参数错误）
            avg_click = stats.get("avg_click_share", 0)
            if isinstance(avg_click, (int, float)) and avg_click > 0:
                if avg_click < 5:
                    ...
                    rating = "高垄断 ❌"
                    color = "red"
                st.markdown(f"📊 平均点击份额 **{avg_click:.2f}%** → 评级：<span style='color:{color};font-weight:bold;'>{rating}</span>", unsafe_allow_html=True)
            else:
