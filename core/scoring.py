...
def calculate_10d_scores(cluster: Dict[str, Any], keyword_df: pd.DataFrame = None,
    ...
    if asin_df is not None and not asin_df.empty:
        matched = asin_df[asin_df['asin'].isin([k.get('clickedAsin') for k in cluster.get('items', [])])]
        if not matched.empty:
    ...
    weights = {
        'market_size': 0.25, 'conversion': 0.20, 'competition': 0.25,
        'growth': 0.15, 'profit': 0.15, 'brand_monopoly': 0.05,
        'new_product': 0.05, 'modifier_acceptance': 0.05,
        'price_elasticity': 0.05, 'seasonality': 0.05
    }
    ...
    weight_level, weight_factor = assign_weight_level(
        cluster.get('avg_rank', 999999), 100000
    )
    ...
    if final_score >= 8.0:
        ...
        grade, grade_label = "A", "值得进入"
    elif final_score >= 6.0:
        grade, grade_label = "B", "谨慎考虑"
    elif final_score >= 4.5:
        grade, grade_label = "C", "一般不推荐"
    else:
        grade, grade_label = "D", "不推荐"
